...
func main() {
	...
	fmt.Printf(strings.Repeat("|%-11s", 4)+"|\r\n", "名称", "代码", "涨幅", " 换手")
	fmt.Println(strings.Repeat("-", 102))
	...
	for _, data := range datas {
		...
		// 排除 ST, 创业板, 科创板
		if arr[4] != "-" && !strings.HasPrefix(arr[2], "*ST") && !strings.HasPrefix(arr[2], "ST") && !strings.HasPrefix(arr[1], "300") && !strings.HasPrefix(arr[1], "688") {
			tmpStrFloat := arr[15]
			...
			tmpStrV := arr[5]
			tmpV2, err := strconv.ParseFloat(tmpStrV, 64)
			if err != nil {
				continue
			}
	...
	fmt.Println("上涨", zCount, float64(zCount)/float64(count)*100)
	fmt.Println("下跌", fCount, float64(fCount)/float64(count)*100)
}
