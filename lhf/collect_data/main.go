...
func _write_info_to_csv (_stocks map[string]*StrockInfo) bool {
	...
	fs.WriteString("股票代码,股票名称,市盈率,市净率,最新价,换手率,量比\r\n")
	for _, _stock := range _stocks {
		_s := fmt.Sprintf("%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f", _stock.Ticker, _stock.Name, _stock.Shiying, _stock.Shijing, _stock.Price, _stock.Huanshou, _stock.Liangbi)
		fs.WriteString(_s+"\r\n")
	}
	return true
}
